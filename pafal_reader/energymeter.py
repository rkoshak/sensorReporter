#!/bin/python
"""Script to read out current value of D0-Counter Pafal 20ec3gr.
...
# Startsequence
# 0.0.0(71786316)		- Zaehlernr
# 0.0.1(PAF)			- Eigentumsnr
# F.F(00)				- ???
# 0.2.0(1.27)			- Firmware
# 1.8.0*00(048162.13)	-	Bezug
# 2.8.0*00(035411.79) -	Lieferung
# C.2.1(000000000000)(                                                )	- ???
# 0.2.2(:::::G11)!	- Schaltuhrenprogramm + Endzeichen
# q		- Block check
_DATA_EOT_SIGNAL = "!"
_DATA_STARTSEQUENCE = b"\x02"


class pafalConnector(object):
  ...
  def _setupDevice(self):
    ...
    self.serialDevice = serial.Serial(
        ...
        # xonxoff = False, rtscts = False, dsrdtr = False,  # No handshakes...
        timeout = 1)   # 1s read time out
  ...
  def readData(self, requestedIDs):
    ...
    Shall not be called with higher frequency than 1 call / second.
    Not thread-safe. Blocking.

    If an error occurs, an empty dict is handed back.

    requestedIDs = {
      "ID": [asFloat as boolean]
    }
